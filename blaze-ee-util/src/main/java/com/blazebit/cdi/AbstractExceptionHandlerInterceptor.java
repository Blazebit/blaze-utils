 ...
 * is annotated with the #{@link Cleanup} annotation and the declared name
 * will be executed in case, an exception occurred. An exception handling
 * element can override the behavior for a different exception type, by also
 * declaring a cleanup reference via cleanupName attribute.
 * If the cleanup in the exception handling annotation cannot be found but the
 * cleanup of the exception handler is available, then the cleanup of the
 * exception handler is invoked.
 * <br/>
 * With the ExceptionMessage annotation
 * you can define a default message for the particular exception. If you define
 * the default Message in your ExceptionHandling Annotation as a default value
 * for the Enum parameter. Then the default message of the exceptionMessage
 * annotation will be used. Otherwise define the Enum in the Exception handling
 * annotation and it will be used.
 * <br/>
 * If you implement an ExceptionHandler
 * please take a look at CoreExceptionHandler and take this implementation as an
...
public abstract class AbstractExceptionHandlerInterceptor<T extends Annotation, V extends Annotation, M extends Enum<M>> {
    ...
    public Object handleError(InvocationContext ic) throws Exception {
        ...
        try {
            ...
            if (!handleException(ic.getMethod().getAnnotation(clazz), ic, targetObj, t)) {
                // Class level exception handling if not handled by method level
                if (!handleException(AnnotationUtil.findAnnotation(targetObj.getClass(), clazz), ic, targetObj, t)) {
     ...
     *
     * @param handler the handler, class or method level
     * @param ic the InvocationContext
     * @param t the thrown throwable
     * @throws Exception if an error occurs the handling
     */
    ...
    private boolean handleException(T handler, InvocationContext ic, Object targetObj, Throwable t) throws Exception {
        ...
        if (handler != null) {
            ...
            for (Object exHandle : handlings) {
                if (t.getClass().equals((Class<? extends Exception>) exHandle.getClass().getMethod("exception").invoke(exHandle))) {
//                    logError(log, ic.getMethod().getDeclaringClass().getSimpleName(), ic.getMethod().getName(), t);
                    Enum<M> message = (Enum<M>) exHandle.getClass().getMethod("message").invoke(exHandle);
     ...
     * handling.
     *
     * @param clazz the class of the bean to get the methods from
     * @param target the target on which the method is invoked
     * @param cleanupName the name of the cleanup method
     * @return true if the cleanup method were found and invoked
     * @throws Exception if an reflection specific exception occurs
     */
     ...
     *
     * @param ex the thrown Throwable
     * @param message the message corresponding to the Throwable
     */
