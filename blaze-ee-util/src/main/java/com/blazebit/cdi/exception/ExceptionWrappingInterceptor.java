 ...
 * </code>
 * 
 * RuntimeExceptionA will be wrapped into MyExceptionA
 * RuntimeExceptionB will be wrapped into MyExceptionB
 * Every other Exception, that is not instance of MyExceptionA, MyExceptionB or
 * MyExceptionC, will be wrapped into MyExceptionC
 * 
 ...
 * 
 * If you would specify @ExceptionWrap(wrapper=MyExceptionC.class) as the first
 * element of the ExceptionWrapping annotation, then every exception that is
 * not declared in the throws clause will be wrapped into a MyExceptionC!
 * 
 * @since 0.1.2
...
public class ExceptionWrappingInterceptor implements Serializable {
    ...
    public Object errorLogging(InvocationContext ic) throws Exception {
        ...
        ExceptionWrapping wrappingAnnotation = AnnotationUtil.findAnnotation(m, targetClass, ExceptionWrapping.class);
        ...
        try {
            ...
            if(doWrapping){
                // Check if the exception that was thrown is declared in the throws
                ...
                if (doWrapping) {
                    // If a wrapping should be applied, iterate through exception
                    ...
                    for (ExceptionWrap wrap : wraps) {
                        ...
                        for (Class<? extends Throwable> source : sourceClasses) {
                            // When the thrown exception is instance of an exception
                            // wrap source class, create a new exception instance
                            // with the thrown exception as cause
                            if (source.isInstance(t1)) {
                                try {
                                    e = wrapperClass.getConstructor(Throwable.class).newInstance(t1);
                                } catch (Throwable t2) {
                                    // Declared wrapper exception has no constructor
                                    // with a throwable as param
                                    Exception ex1 = null;

                                    // Cast to or wrap the throwable into a new 
                                    // exception because we may not throw Throwable
