...
public class CatchHandlerInterceptor implements Serializable {
    ...
    public Object handle(InvocationContext ic) throws Exception {
        ...
        CatchHandler exceptionHandlerAnnotation = AnnotationUtil.findAnnotation(m, targetClass, CatchHandler.class);
        ...
        try{
            ...
            if(exceptionHandlingAnnotations.length > 0){
                for(CatchHandling handling : exceptionHandlingAnnotations){
                    if(handling.exception().isInstance(t)){
                        ...
                        if(!handling.cleanup().equals(NullClass.class)){
                            invokeCleanups(targetClass, targetObject, handling.cleanup());
                        }
                ...
                // Handle the default exception type if no handlings are declared
                if(exceptionHandlerAnnotation.exception().isInstance(t)){
                    ...
                    if(!exceptionHandlerAnnotation.cleanup().equals(NullClass.class)){
                        invokeCleanups(targetClass, targetObject, exceptionHandlerAnnotation.cleanup());
                    }
                }
            }
    ...
    /**
     * This method should populate the given throwable to a handler that will
     * do the appropriate exception handling.
     * The default implementation will populate the throwable via CDI-Event.
     * 
