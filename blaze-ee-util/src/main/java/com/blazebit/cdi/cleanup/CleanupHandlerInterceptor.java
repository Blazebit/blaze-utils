...
/**
 * Invokes cleanup methods after the invocation of a method.
 * The specified cleanup with the cleanup name declared in CleanupHandling is
 * used for the declared exception. If CleanupHandling does not specify a
 * cleanup name, the cleanup name of the CleanupHandler is used.
 * Cleanup methods of CleanupHandling elements that have the flag always set to
 * true, are always invoked.
 *
...
public class CleanupHandlerInterceptor implements Serializable {
    ...
    public Object cleanup(InvocationContext ic) throws Exception {
        ...
        CleanupHandler cleanupHandlerAnnotation = AnnotationUtil.findAnnotation(m, targetClass, CleanupHandler.class);
    ...
    private void handleCleanups(Object target, CleanupHandler handler, Throwable t) throws Exception {
        ...
        if (handlings.length > 0) {
            ...
            for (CleanupHandling handling : handlings) {
                ...
                }else if (t != null && t.getClass().equals(handling.exception())) {
                    doInvokeCleanup(target, handler, handling);
                }
            }
        } else {
