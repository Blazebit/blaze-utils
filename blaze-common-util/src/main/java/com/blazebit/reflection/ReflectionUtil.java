...
public class ReflectionUtil {
     ...
     * </code>
     *
     * @param targetClazz the class to check wether it is a subtype of the
     * supertype or not
     * @param superType the supertype class to search on targetClazz
     ...
     * method is equal to calling #{@link ReflectionUtil#getField(java.lang.Class, java.lang.String)
     * }
     * with a null check and finally return the type via getType().
     *
    ...
    public static Class<?> getResolvedFieldType(Class<?> clazz, Field f) {
        ...
        if (f.getGenericType() instanceof TypeVariable<?>) {
            return resolveTypeVariable(clazz, (TypeVariable<?>) f.getGenericType());
        }
    ...
    public static Class<?>[] resolveTypeArguments(Class<?> concreteClass, ParameterizedType parameterizedType) {
        ...
        for (int i = 0; i < argumentTypes.length; i++) {
            if (argumentTypes[i] instanceof TypeVariable<?>) {
                resolvedClasses[i] = resolveTypeVariable(concreteClass, (TypeVariable<?>) argumentTypes[i]);
            } else {
     ...
     *
     * @param concreteClass The class which is used to resolve the type. The
     * type for the type variable must be bound in this class or a superclass.
     * @param typeVariable The type variable to resolve.
     * @return The resolved type as class.
     * @throws IllegalArgumentException Is thrown when the concrete class is not
     * a subtype of the class in which the type variable has been declared, or
     * when the type for the type variable is not bound within the hierarchy of
     * the concrete class.
     */
     ...
     * declared.
     *
     * @param typeVariable The type variable for which the position should be
     * retrieved
     ...
     *
     * @param genericDeclartion The generic declartion type in which to look for
     * the type variable
     ...
     * Calling this method is equal to calling #{@link ReflectionUtil#getMethod(java.lang.Class, java.lang.String)
     * }
     * with a null check and finally return the type via getReturnType().
     *
     * @param clazz The class within to look for the method with the given
     * method name
    ...
    public static Class<?> getResolvedMethodReturnType(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        return getResolvedMethodReturnType(clazz, getMethod(clazz, methodName, parameterTypes));
    }
    ...
    public static Class<?> getResolvedMethodReturnType(Class<?> clazz, Method m) {
        ...
        if (m.getGenericReturnType() instanceof TypeVariable<?>) {
            return resolveTypeVariable(clazz, (TypeVariable<?>) m.getGenericReturnType());
        }
    ...
    public static Class<?>[] getResolvedMethodReturnTypeArguments(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        return getResolvedMethodReturnTypeArguments(clazz, getMethod(clazz, methodName, parameterTypes));
    }
    ...
    public static Class<?>[] getResolvedMethodParameterTypes(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        return getResolvedMethodParameterTypes(clazz, getMethod(clazz, methodName, parameterTypes));
    }
    ...
    public static Class<?>[] getResolvedMethodParameterTypes(Class<?> clazz, Method m) {
        ...
        for (int i = 0; i < genericParameterTypes.length; i++) {
            if (genericParameterTypes[i] instanceof TypeVariable<?>) {
                parameterTypes[i] = resolveTypeVariable(clazz, (TypeVariable<?>) genericParameterTypes[i]);
            } else {
    ...
    public static Class<?>[] getResolvedMethodExceptionTypes(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        return getResolvedMethodExceptionTypes(clazz, getMethod(clazz, methodName, parameterTypes));
    }
    ...
    public static Class<?>[] getResolvedMethodExceptionTypes(Class<?> clazz, Method m) {
        ...
        for (int i = 0; i < genericExceptionTypes.length; i++) {
            if (genericExceptionTypes[i] instanceof TypeVariable<?>) {
                exceptionTypes[i] = resolveTypeVariable(clazz, (TypeVariable<?>) genericExceptionTypes[i]);
            } else {
    ...
    public static MethodParameter[] getMethodParameters(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        return getMethodParameters(clazz, getMethod(clazz, methodName, parameterTypes));
    }
    ...
    public static MethodException[] getMethodExceptions(Class<?> clazz, String methodName, Class<?>... parameterTypes) {
        return getMethodExceptions(clazz, getMethod(clazz, methodName, parameterTypes));
    }
    ...
    private static boolean isGetter(Method m) {
        return m != null && !void.class.equals(m.getReturnType()) && m.getParameterTypes().length == 0;
    }
    ...
    private static boolean isSetter(Method m) {
        return m != null && m.getReturnType().equals(void.class) && m.getParameterTypes().length == 1;
    }
}
