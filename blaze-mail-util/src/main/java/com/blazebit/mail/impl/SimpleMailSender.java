...
public class SimpleMailSender implements MailSender {
    ...
    public Session createMailSession(String host, Integer port, String username, String password) {
        ...
        if (port != null) {
            ...
            props.put(transport.getPortProperty(), String.valueOf(transport.getDefaultPort()));
        }
        ...
        if (password != null) {
            ...
            return Session.getInstance(props, new SimplePasswordAuthenticator(username, password));
        } else {
    ...
    private Message prepareMessage(Mail email) throws MessagingException, UnsupportedEncodingException {
        ...
        message.setFrom(new InternetAddress(email.getFrom().getAddress(), email.getFrom().getName()));
    ...
    private void setTexts(Mail email, MimeMultipart multipartAlternativeMessages) throws MessagingException {
        ...
        if (email.getHtml() != null) {
            ...
            messagePartHTML.setContent(email.getHtml(), "text/html; charset=\"UTF-8\"");
