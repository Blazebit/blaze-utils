...
public abstract class AbstractHttpInvokerJob extends AbstractJob {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            /*
             * Build up the post parameter string for the request
             * Only use parameters that are not explicitly defined for this job
             */
            StringBuilder data = new StringBuilder();
            
            for (Map.Entry<String, Object> entry : getUndefinedParameters(map).entrySet()) {
                if (data.length() != 0) {
                    data.append("&");
                }
            ...
            /*
             * Handle the response of the request, default is print to System.out
             */
            ...
            throw new JobExecutionException("Error occured during the invocation of the URL", ex, false);
        } finally {
            if (is != null) {
                try {
                    is.close();
    ...
    public List<JobParameter> getParameters() {
        List<JobParameter> params = new ArrayList<JobParameter>(super.getParameters());
        params.add(new JobParameter("url", true, String.class));
        params.add(new JobParameter("urlSuffix", false, String.class));
        return params;
    }
    
    protected abstract URLConnection createConnection(String url, String parameterString) throws IOException;
