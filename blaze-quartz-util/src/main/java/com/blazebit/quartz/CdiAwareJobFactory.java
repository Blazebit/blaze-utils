            ...
            for (Class<?> interfaceClass : jobClass.getInterfaces()) {
                if (CdiAwareJob.class.equals(interfaceClass)) {
                    ...
                    try {
                        ...
                        log.error("BeanManager could not be found! The reason for this can be that the thread which creates the job is not managed by the application server! CDI is disabled for jobs!", ex);
                    }
